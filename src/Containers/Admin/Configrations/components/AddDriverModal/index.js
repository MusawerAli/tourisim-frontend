...
const AddDriverModal = (props) => {
  ...
  const { modal, setModal, updateData, driverErrorData,
  ...
  const onAddDriver = (type) => {
    ...
    if (type) {
      debugger;
      props.updateDriver(updateData.user.user_uuid, data, setModal);
    } else {
      props.addDriver(data, setModal);
    }
  };
  ...
  return (
    <>
      {showMessage ? (
        <SweetAlert
          title="Driver Created Successfull"
      ...
      <Modal isOpen={modal}>
        <ModalBody>
          <div className="userModalHeading pb-4 textColor">
            <h5>{`${updateData ? "Update" : "Create New"} Driver`}</h5>
            <div className="pl-5">
              <img
                src={closeIcon}
          ...
          {request_error_response&&request_error_response?.name && (
            ...
            <Col md="6">
              ...
              {driverErrorData&&driverErrorData.errors?.name && (
                  <div
                    className="text-center"
                    style={{ color: "red", float: "left" }}
                  >
                    <p>{driverErrorData.errors?.name}</p>
                  </div>
                )}
            </Col>
            <Col md="6">
              ...
              <FormGroup>
                ...
                />
                {driverErrorData.plan_type_name_en && (
                  <div
                    className="text-center"
                    style={{ color: "red", float: "left" }}
                  >
                    <p>{driverErrorData.plan_type_name_en}</p>
                  </div>
                )}
              </FormGroup>
            </Col>
          ...
          <Row className="pt-2">
            <Col md="6">
              ...
              {driverErrorData&&driverErrorData.errors?.email && (
                  <div
                    className="text-center"
                    style={{ color: "red", float: "left" }}
                  >
                    <p>{driverErrorData.errors?.email}</p>
                  </div>
                )}
            </Col>
            <Col md="6">
              ...
              <FormGroup>
                ...
                />
                {driverErrorData&&driverErrorData.errors?.mobile_number && (
                  <div
                    className="text-center"
                    style={{ color: "red", float: "left" }}
                  >
                    <p>{driverErrorData.errors?.mobile_number}</p>
                  </div>
                )}
              </FormGroup>
            </Col>
          ...
          <Row className="pt-3">
            <Col md="12" className="justify-content-center d-flex">
              <Button
                ...
                disabled={disable}
                onClick={() => onAddDriver(updateData ? 1 : 0)}
              >
...
const mapStateToProps = (state) => ({
  driverErrorData: state.commonReducer.driverErrorData,
...
const mapDispatchToProps = (dispatch) => ({
  addDriver: (data, setModal, setShowMessage) =>
    dispatch(addDriver(data, setModal, setShowMessage)),
  updateDriver: (id, data, setModal,setShowMessage) =>
    dispatch(updateDriver(id, data, setModal,setShowMessage)),
});
